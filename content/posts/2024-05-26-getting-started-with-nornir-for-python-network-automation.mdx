---
title: "Getting Started with Nornir for Python Network Automation"
date: "2024-05-26"
excerpt: "Nornir is a Python library designed for network automation tasks. It allows Network Engineers to manage and automate their network devices using Pytho..."
description: "Nornir is a Python library designed for network automation tasks. It allows Network Engineers to manage and automate their network devices using Python. Unlike tools like Ansible that use domain-speci..."
...
![](https://cdn-images-1.medium.com/max/800/0*PFhcuW7VkK4wJa53)
...
*Networking labs, providing a sandbox for testing, learning, and refining skills. In this blog, we’ll delve into the…*medium.com](https://medium.com/@sydasif78/setting-up-a-network-lab-with-gns3-60f908dd9863 "https://medium.com/@sydasif78/setting-up-a-network-lab-with-gns3-60f908dd9863")
...
* **Output**: The output from print\_result shows a structured view of the task execution. For each device (R1 and sw1), it indicates the task name (say\_hello), the status (changed: False), and the message returned by the task (Hello, Nornir). This format makes it easy to understand what happened during the script’s execution and to troubleshoot if necessary.
...
### Introduction to the nornir\_netmiko Plugin
...
Installing the nornir\_netmiko Plugin:

```
pip install nornir_netmiko
```

Sending Commands with nornir\_netmiko:
...
In this script, we’re leveraging the nornir\_netmiko plugin, particularly the `netmiko_send_command` function, to execute commands on network devices. After initializing Nornir, we call `nr.run`, passing in `netmiko_send_command` as the task. We specify the command we want to run on our devices with `command_string='show ip interface brief | excl down'`.
...
In this script, the napalm\_get task is used with the facts-getter to retrieve basic information about the devices, such as vendor, model, serial number, and uptime.
...
Nornir, integrated with NAPALM and supported by plugins like nornir\_netmiko and nornir\_utils, offers a robust solution for network automation tasks. This combination enhances automation capabilities, allowing for efficient retrieval and configuration of network device data. With NAPALM’s multivendor support and common API, along with Nornir’s powerful task management, network automation becomes more manageable, scalable, and tailored to your specific needs. This guide has demonstrated how to set up Nornir, create basic and advanced scripts, utilize host-specific data for dynamic configurations, and leverage plugins to efficiently manage and configure network devices.

[**Network-Autmotion / nornir\_lab · GitLab**](https://gitlab.com/network-autmotion/nornir_lab "https://gitlab.com/network-autmotion/nornir_lab")